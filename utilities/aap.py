...
AWX_PROJECTS_STORAGE_CLASS: str = "ocs-storagecluster-cephfs"  # CephFS required — AWX projects PVC needs RWX
...
def deploy_awx_via_helm() -> None:
    ...
    _run_shell(f"helm repo add {AWX_HELM_REPO_NAME} {AWX_HELM_REPO_URL}")
...
def create_awx_instance(
    ...
    Applies the AWX CR with Route ingress and CephFS storage for both
    postgres and projects volumes. Skips if an AWX instance already exists.
    ...
    """
    try:
        awx_resource = ocp_admin_client.resources.get(api_version="awx.ansible.com/v1beta1", kind="AWX")
        awx_resource.get(name=AWX_INSTANCE_NAME, namespace=namespace)
...
def wait_for_awx_ready(
    ...
    while time.time() < deadline:
        ...
        ready_pods = [
            pod
            for pod in pods
            if any(pod.name.startswith(prefix) for prefix in expected_prefixes)
            and pod.instance.status.phase == "Running"
        ]
        if len(ready_pods) >= len(expected_prefixes):
            LOGGER.info(f"All {len(ready_pods)} AWX pods are Running")
            return
        time.sleep(15)
...
def create_aap_token_secret(
    ...
    """
    LOGGER.info(f"Creating AAP token secret '{AAP_TOKEN_SECRET_NAME}' in namespace '{namespace}'")
...
def teardown_awx() -> None:
    ...
    try:
        ...
        LOGGER.warning("Helm uninstall failed (may already be removed)")
    try:
        _run_shell(f"oc delete namespace {AWX_NAMESPACE} --wait=false")
    except subprocess.CalledProcessError:
        LOGGER.warning(f"Failed to delete namespace '{AWX_NAMESPACE}'")
